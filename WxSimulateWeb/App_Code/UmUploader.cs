...
namespace WxSimulateWeb.ueditor.net
{
    /// <summary>
    /// UEditor编辑器通用上传类
    /// </summary>
      ...
      */
        public Hashtable upFile(HttpContext cxt, string pathbase, string[] filetype, int size)
        {
            ...
            try
            {
                ...
                //目录创建
                createFolder(cxt.Request["USER"]);

                //格式验证
                if (checkType(filetype))
                {
                    state = "不允许的文件类型，只能上传图片";
                }
                //大小验证
                if (checkSize(size))
                {
                    state = "文件大小超出微信平台限制：300KB";
                }
                //保存图片
                if (state == "SUCCESS")
                {
                    ...
                    URL = pathbase.Substring(1) + filename;
                }
            }
            catch (Exception ex)
            {
    ...
    */
         ...
         * 按照日期自动创建存储文件夹
         */
        private void createFolder(string uid)
        {
            if (!Directory.Exists(uploadpath))
            {
                WxSimulateWebHelper.UserDirectory.Creat(uploadpath, uid);
